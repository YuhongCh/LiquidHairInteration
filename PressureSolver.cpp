...
void CGSolver<2>::Init() {
	...
	for (Integer level = 0; level < m_maxLevel; ++level) {
		...
		m_r[level].Fill(0.0);
		m_z[level].Fill(0.0);
		m_Adiag[level].Fill(0.0);
		m_Acoef[level].Fill(Vector4::Zero());
	}
}
...
Scalar CGSolver<2>::GetAx(const Array2<Scalar>& grid, Integer xi, Integer yi, Integer level) const {
	Integer width = m_grid.dimension.x() / (1 << level);
	Integer height = m_grid.dimension.y() / (1 << level);
#ifdef _DEBUG
...
void CGSolver<2>::Solve(const Scalar& liquidDensity, const Scalar& dt) {
	...
	m_oldRZ = Array2<Scalar>::Reduce(m_r[0], m_z[0]);
	Scalar residual =  std::sqrt(Array2<Scalar>::Reduce(m_r[0], m_r[0])) / numLiquidCell;
	...
	for (Integer iter = 0; iter < m_maxIteration; ++iter) {
		...
		ApplyPreconditioner();
		m_newRZ = Array2<Scalar>::Reduce(m_r[0], m_z[0]);
...
Scalar CGSolver<3>::GetApproxLiquidPhi(Integer xi, Integer yi, Integer zi, Integer level) const {
	...
	return MathUtils::Trilerp(
		m_grid.liquidPhi(baseX, baseY, baseZ),
		m_grid.liquidPhi(baseX + 1, baseY, baseZ),
		m_grid.liquidPhi(baseX, baseY + 1, baseZ),
		m_grid.liquidPhi(baseX + 1, baseY + 1, baseZ),
		m_grid.liquidPhi(baseX, baseY, baseZ + 1),
		m_grid.liquidPhi(baseX + 1, baseY, baseZ + 1),
		m_grid.liquidPhi(baseX, baseY + 1, baseZ + 1),
		m_grid.liquidPhi(baseX + 1, baseY + 1, baseZ + 1),
		0.5, 0.5, 0.5
	);
}
...
Scalar CGSolver<3>::GetApproxWx(Integer xi, Integer yi, Integer zi, Integer level) const {
	...
	Integer baseY = factor * yi + (1 << (level - 1)) - 1;
	Integer baseZ = factor * zi + (1 << (level - 1)) - 1;
	return MathUtils::Bilerp(
		m_grid.liquidWx(baseX, baseY, baseZ),
		m_grid.liquidWx(baseX, baseY + 1, baseZ),
		m_grid.liquidWx(baseX, baseY, baseZ + 1),
		m_grid.liquidWx(baseX, baseY + 1, baseZ + 1),
...
Scalar CGSolver<3>::GetApproxWy(Integer xi, Integer yi, Integer zi, Integer level) const {
	...
	Integer baseZ = factor * zi + (1 << (level - 1)) - 1;
	return MathUtils::Bilerp(
		m_grid.liquidWy(baseX, baseY, baseZ),
		m_grid.liquidWy(baseX + 1, baseY, baseZ),
		m_grid.liquidWy(baseX, baseY, baseZ + 1),
		m_grid.liquidWy(baseX + 1, baseY, baseZ + 1),
		Scalar(0.5), Scalar(0.5)
	);
}
...
Scalar CGSolver<3>::GetApproxWz(Integer xi, Integer yi, Integer zi, Integer level) const {
	...
	return MathUtils::Bilerp(
		m_grid.liquidWz(baseX, baseY, baseZ),
		m_grid.liquidWz(baseX + 1, baseY, baseZ),
		m_grid.liquidWz(baseX, baseY + 1, baseZ),
		m_grid.liquidWz(baseX + 1, baseY + 1, baseZ),
		Scalar(0.5), Scalar(0.5)
	);
}
...
#pragma omp parallel for
	for (Integer level = 0; level < m_maxLevel; ++level) {
		...
		m_z[level].Fill(0.0);
		m_Adiag[level].Fill(0.0);
		m_Acoef[level].Fill(VectorX<6>::Zero());
	}
}
...
Scalar CGSolver<3>::GetAx(const Array3<Scalar>& grid, Integer xi, Integer yi, Integer zi, Integer level) const {
	Vector3i dimension = m_grid.dimension / (1 << level);
#ifdef _DEBUG
...
#pragma omp parallel for
	...
	m_oldRZ = Array3<Scalar>::Reduce(m_r[0], m_z[0]);
	Scalar residual = std::sqrt(Array3<Scalar>::Reduce(m_r[0], m_r[0]));
...
#pragma omp parallel for
		...
		ApplyPreconditioner();
		m_newRZ = Array3<Scalar>::Reduce(m_r[0], m_z[0]);
		m_beta = m_newRZ / m_oldRZ;
